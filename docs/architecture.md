...
1. 客户端先通过 `chat-service` 创建会话。
2. 客户端连接 `realtime-service` 的 WebSocket。
3. 发送 `message.send` 后，`realtime-service` 通过 gRPC 调用 `chat-service` 写入消息。
4. 消息写入成功后发到 Redis 频道，由所有 `realtime-service` 实例广播给同会话连接。
5. 业务 HTTP API 统一在 `gateway-service` 处理，再通过 gRPC 调 `chat-service`、`auth-service`、`admin-service`。
6. `chat/auth/admin/realtime` 启动后将自身地址注册到 `etcd`，由租约续期维持在线状态。
