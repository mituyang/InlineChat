...
func (d *DynamicClient) GetSiteAIConfig(ctx context.Context, siteID string) (*SiteAIConfig, error) {
	client, err := d.clientForCall(ctx)
	if err != nil {
		return nil, err
	}
	resp, err := client.GetSiteAIConfig(ctx, siteID)
	if err == nil {
		return resp, nil
	}
	...
	return client.GetSiteAIConfig(ctx, siteID)
}
