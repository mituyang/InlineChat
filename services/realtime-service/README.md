...
- 通过 `chat-service` 完成会话校验、历史补拉与消息写入。
- 在握手阶段通过 `auth-service` 二次校验客服 token，并通过 `admin-service` 校验站点状态。
- 订阅 Redis 事件并向同会话连接广播。

## 端口与探针
- 默认 HTTP 端口：`8203`（`HTTP_PORT`）
- 健康检查：`GET /healthz`
- 就绪检查：`GET /readyz`（检查 Redis + chat/auth/admin 发现结果）
- 指标：`GET /metrics`
...
| `CHAT_GRPC_DIAL_TIMEOUT_SEC` | `8` | 否 | realtime 侧 gRPC 建连超时（chat/auth/admin 客户端共享） |
| `CHAT_GRPC_CALL_TIMEOUT_SEC` | `8` | 否 | realtime 侧 gRPC 调用超时（chat/auth/admin 客户端共享） |
...
| `SERVICE_INSTANCE_ID` | 空 | 否 | 实例 ID（缺省自动生成） |
...
- 连接阶段返回 `site is unavailable` / `site is not active`：优先检查 `admin-service` 与站点状态。
