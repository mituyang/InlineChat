...
// NewDynamic 构建带服务发现能力的 admin gRPC 客户端。
...
// GetSiteBySiteID 优先使用当前连接，请求失败且可重试时自动刷新 endpoint 后重试一次。
func (d *DynamicClient) GetSiteBySiteID(ctx context.Context, siteID string) (*Site, error) {
	client, err := d.clientForCall(ctx)
	if err != nil {
		return nil, err
	}

	resp, err := client.GetSiteBySiteID(ctx, siteID)
	if err == nil {
		return resp, nil
	}
	...
	return client.GetSiteBySiteID(ctx, siteID)
}
...
func (d *DynamicClient) clientForCall(ctx context.Context) (*Client, error) {
	...
	client := d.currentClient()
	if client == nil {
		return nil, fmt.Errorf("admin grpc client is unavailable")
	}
	return client, nil
}
